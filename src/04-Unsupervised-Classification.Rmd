---
title: "04-Unsupervised Classification"
output:
...
In unsupervised machine learning, potential matching pairs are identified based on weights.  Each record in a comparative pair gets a weight based on its proximity to its twin, in turn based on the similarity of values in their variables.  Values above a certain threshold is considered to be an identical pair.  

Since supervised classification depends purely on the values falling between a set of threshold values, exceptionally certain pairs tend to slip through as duplicates even while being unique records. People living at the same address, with the same last name are typical cases in point.  While their similarity score remain exceptionally high, they are not duplicate records and should be treated as exceptions. For this reason, unsupervised classification tends to come up with a high number of "false" positives. This method is included here for completeness of the experimental framework; but not detailed.  
